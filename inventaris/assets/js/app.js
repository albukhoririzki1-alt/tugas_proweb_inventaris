...
async function loadProfil() {
  ...
  try {
    ...
    document.getElementById('profil-username').value =
      data.username || '';
    ...
    const initialEl =
      document.getElementById('profil-initial');

    const preview =
      document.getElementById('profil-preview');

    if (data.foto) {

      preview.innerHTML = `
        <img
          src="${data.foto}?t=${Date.now()}"
          alt="Foto Profil"
          style="
            width:100%;
            height:100%;
...
function previewProfilFoto(input) {
...
async function simpanProfil() {
  ...
  const passwordLamaEl =
    document.getElementById(
      'profil-password-lama'
    );

  const passwordLama = passwordLamaEl ? passwordLamaEl.value : '';
  ...
  if (!passwordLamaEl) {
    console.error('Element profil-password-lama tidak ditemukan!');
    toast(
      'Error: Field password lama tidak ditemukan. Silakan refresh halaman.',
      'error'
    );
    return;
  }

  // Validasi password lama wajib diisi untuk verifikasi
  ...
  // Validasi konfirmasi password jika mengisi password baru
